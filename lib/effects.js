...
module.exports = function(opts) {
	...
	animate((function() {
		...
		var tween = tweener(function(k, lastK) {
			...
			bodybg.style.opacity = (1 - k) * 0.7 + 0.3;

			// Rotations
			prefixed(header.style, "transform", rotateX(k * 90));
			prefixed(projects.style, "transform", rotateX((1 - k) * -90));

			// Body perspective
			prefixed(document.body.style, "perspective-origin-y",
				(height() * 2) - scroll() + "px");
		});
...
function isSimple() {
	return isMobile(window.navigator.userAgent) || ! isMinWidth(848);
}
